# Member-state positions

> One block per national-government non-paper / delegation stream. Council WK delegation-comment
> streams do not exist yet for this file (working-party work has not started); until they do, this
> page tracks non-papers and national-parliament scrutiny.

## Netherlands

**Non-paper "An Integrated Approach for the EU Tech Sovereignty Package"** (The Hague, 17 April 2026;
tabled around the informal Telecom Council of 20 April 2026) —
[register](../sources/README.md) · [Tweede Kamer 2026D18815](https://www.tweedekamer.nl/downloads/document?id=2026D18815)

- Core idea: organise the package around a **"Sovereign Tech Stack"** with a self-reinforcing
  demand–supply loop across the stack's layers (raw materials/hardware/infrastructure up to software,
  data and applications), with **open source as cross-cutting enabler**; builds on the "EuroStack
  vision of European digital sovereignty".
- Technological sovereignty is **"not binary"**; explicitly supports earlier **German and Danish**
  non-papers on CADA (those texts are not yet registered here — watch-item).
- For CADA specifically: focus on a sovereign and sustainable EU cloud/AI ecosystem **and** actively
  creating markets for trustworthy and open technologies.

**National context:**

- The cabinet now has a State Secretary for Digital Economy and Sovereignty (W.J.M. Aerdts, EZ);
  digital sovereignty is politically live in the Tweede Kamer (debates on dependence on US cloud for
  DigiD, Belastingdienst, etc.; committee: [Digitale Zaken](https://www.tweedekamer.nl/kamerleden_en_commissies/commissies/diza)).
- A formal **BNC-fiche** (dossier series 22112) on CADA/Chips Act 2.0 had not appeared as of
  10 June 2026; expected ~6 weeks after the proposal (≈ mid-July 2026) — tracked via the
  [watchlist](../tracker.yaml).
- The annotated agenda for the formal Telecom Council of 9 June 2026 (Kamerstuk 21501-33) predates the
  package presentation and contains no CADA appraisal.

## D9+ group (incl. NL)

The **D9+ ministerial meeting** (Luxembourg, 8 June 2026 — the 13 most digitally advanced Member
States, incl. NL, DK, IE) discussed "challenges and ambitions relating to digital sovereignty in an
open manner" under its European-competitiveness debate, reported to the TTE Council as AOB —
[ST 9881/26](https://data.consilium.europa.eu/doc/document/ST-9881-2026-INIT/en/pdf). A bloc to watch
for coordinated positions on the sovereignty framework ("open" framing suggests wariness of
protectionist readings).

## Germany / Denmark

Referenced by the NL non-paper as authors of earlier CADA non-papers; texts not yet obtained.
**Watch-item:** register them when they surface (national-parliament records or press mirrors).
